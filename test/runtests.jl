...
@testset "trace" begin
    ...
    update!(tr, 1, stpara..., true, true)
    @test length(tr.states) == 1
    @test tr.states[1] == st
    update!(tr, 2, stpara..., true, false)
    ...
    @test sprint(show, tr) == """
        ...
        """
    update!(tr, 3, stpara..., false, false)
    @test length(tr.states) == 2
    @test update!(nothing, 4, stpara..., false, false) === nothing
end
...
@testset "fit" begin
    ...
    @test sprint(show, MIME("text/plain"), r) == """
        convexfit converged after 2 iterations:
         |f(x) - f(x')| = 0.0e+00 < 1.0e-08: true
         |x - x'| = 0.0e+00 < 1.0e-08: true
    ...
    r = convexfit(A, b, λ=0.5, xtol=0, ftol=eps(Float64), show_trace=true)
